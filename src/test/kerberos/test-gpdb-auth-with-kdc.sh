...
psql "dbname=postgres hostaddr=127.0.0.1 krbsrvname=postgres  host=localhost user=krbtestuser" -c "SELECT version()" | tee

###
# Also test expiration
psql "dbname=template1" -c "ALTER USER krbtestuser valid until '2014-04-10 11:46:00-07'" | tee

# should not be able to connect anymore
echo "Testing connection, with expired user account. Should not succeed"
! psql "dbname=postgres hostaddr=127.0.0.1 krbsrvname=postgres  host=localhost user=krbtestuser" -c "SELECT version()"

psql "dbname=template1" -c "ALTER USER krbtestuser valid until '2054-04-10 11:46:00-07'" | tee
